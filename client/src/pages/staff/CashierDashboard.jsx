...
import { Search, Printer, CheckCircle, Send, XCircle, LayoutDashboard, Clock, Users } from 'lucide-react';
import toast from 'react-hot-toast';
import { motion, AnimatePresence } from 'framer-motion';

const CashierDashboard = () => {
  const { orders, updateOrderStatus, confirmCashPayment } = useStore();
  ...
  const filteredOrders = (orders || []).filter(o => {
    ...
    return o.orderStatus === filter && matchesSearch;
  });
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className="w-72 bg-[#0B1220] p-8 text-white flex flex-col">
        ...
        <nav className="flex-grow space-y-2">
           <button className="w-full flex items-center gap-4 p-4 bg-orange-500 text-white rounded-2xl font-bold text-sm">
        ...
        <div className="pt-6 border-t border-white/10">
           ...
           <p className="font-bold text-sm">Staff Kasir 01</p>
        </div>
      </aside>
      ...
      <main className="flex-grow overflow-auto p-10">
        <header className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-gray-400 mt-1 text-sm">Kelola transaksi dan status pesanan hari ini secara real-time.</p>
          </div>
          <div className="relative w-80">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
            <input 
              type="text" 
              placeholder="Cari Order, Meja, atau Nama..."
          ...
          {['ALL', 'WAITING_CASHIER_PAYMENT', 'WAITING_PAYMENT', 'PAID', 'PREPARING', 'READY', 'COMPLETED'].map(f => (
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${
                filter === f ? 'bg-[#0B1220] text-white shadow-xl' : 'bg-white text-gray-400 hover:bg-gray-50 border border-transparent'
              }`}
            >
              {f.replace(/_/g, ' ')}
            </button>
            ...
            {filteredOrders.map(order => (
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em]">{order.orderNumber}</span>
                    <h3 className="text-xl font-black text-[#0B1220] mt-1">{order.table?.tableNumber}</h3>
                    <p className="text-[11px] font-bold text-orange-500 flex items-center gap-1 mt-1">
                      <Users size={12} /> {order.customerName}
                    </p>
                  </div>
                  <div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${getStatusColor(order.orderStatus)}`}>
                ...
                <div className="bg-gray-50 rounded-[2rem] p-6 mb-6 space-y-3">
                  ...
                  <div className="pt-4 border-t border-gray-200 flex justify-between items-center">
                    ...
                    <span className="font-black text-lg text-orange-500">Rp {order.totalAmount?.toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  {['WAITING_PAYMENT', 'WAITING_CASHIER_PAYMENT'].includes(order.orderStatus) && (
                    <button 
